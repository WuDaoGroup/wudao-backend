...
async def data_dimension_reduction(
    username: str = Form(...),
    method: str = Form(...),
    dimension: int = Form(...),
    target: str = Form(...),
    ...
    pathlib.Path(f"./static/data/{username}/images/explanation").mkdir(
        parents=True, exist_ok=True
    )
    plt.savefig(
        f"./static/data/{username}/images/explanation/reduction_{method}_{dimension}_{target}.png"
    )
...
async def feature_correlation(username: str = Form(...), method: str = Form(...)):
    ...
    plt.savefig(
        f"./static/data/{username}/images/explanation/correlation_feature_{method}.png"
    )
...
async def target_correlation(
    username: str = Form(...), k_number: int = Form(...), target: str = Form(...)
):
    # plt.rcParams['font.sans-serif'] = ['SimHei']  # 中文字体设置-黑体
    # plt.rcParams['axes.unicode_minus'] = False  # 解决保存图像是负号'-'显示为方块的问题
    ...
    plt.subplots(figsize=(24, 16))
    sns.heatmap(
        cm, annot=True, square=False, yticklabels=cols.values, xticklabels=cols.values
    )
    plt.title(
        f"Correlation Matrix of Targets (Top {k_number} related features)", fontsize=18
    )
    pathlib.Path(f"./static/data/{username}/images/explanation").mkdir(
        parents=True, exist_ok=True
    )
    plt.savefig(
        f"./static/data/{username}/images/explanation/correlation_target_{target}_{k_number}.png"
    )
...
async def pairwise_feature_correlation(username: str = Form(...)):
    ...
    plt.savefig(
        f"./static/data/{username}/images/explanation/correlation_feature_pairwise.png"
    )
